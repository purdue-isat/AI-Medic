...
# ---------------------------------------------------
  ...
  for length_index in range(0,len(label_length)):
    if label_length[length_index] <= 0:
      problematic_captions.append(length_index)
  
  for index in problematic_captions:
    if index != len(label_length):
      label_arrays[index] = label_arrays[index+1]
      label_length[index] = label_length[index+1]
    else:
      label_arrays[index] = label_arrays[index-1]
      label_length[index] = label_length[index-1]
# ---------------------------------------------------
...
def main(params):
  ...
  for i,img in enumerate(imgs):
    # load the image
    #I = Image.open(os.path.join(params['images_root'], img['file_path']))
    I = imread(os.path.join(params['images_root'], img['file_path']))
    try:
        #Ir = I.resize((256,256), Image.ANTIALIAS)
