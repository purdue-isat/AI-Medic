...
cmd:option('-transfer_learning',0,'flag telling whether to do transfer learning')
...
if string.len(opt.start_from) > 0 then
  ...
  if(opt.transfer_learning == 1) then
    local cnn_raw = loadcaffe.load(opt.cnn_proto, opt.cnn_model, cnn_backend)
    temp_cnn = net_utils.reset_weights(cnn_raw,protos.cnn)
    protos.cnn = temp_cnn
  end
...
local best_score
local losslist = {}
local lossfilename = "DaMICa/losses.txt"
local myprecision = 5
local fstr = "%."..tostring(myprecision).."f\n"
while true do  

  -- eval loss/gradient
  local losses = lossFun()
  if iter % opt.losses_log_every == 0 then loss_history[iter] = losses.total_loss end
  losslist[#losslist+1] = losses.total_loss
  print(string.format('iter %d: %f', iter, losses.total_loss))

  -- save checkpoint once in a while (or on final iteration)
  if (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters) then

    file = io.open (lossfilename, "w+")
    for i = 1, #losslist do
      file:write( string.format(fstr, losslist[i]))
    end
    io.close(file)
